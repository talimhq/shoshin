class Teacher::ExerciseFormsController < TeacherController
  ...
  def new
    @exercise_form = current_user.teacher_exercise_forms.new(exercise: @exercise)
  end
  ...
  def authorize_exercise_form
    @exercise_form = TeacherExerciseForm.includes(exercise: :questions).find(params[:id])
    redirect_to root_url unless current_user == @exercise_form.teacher
  end
